# Review gates — the room-artifact gauntlet

Run before any external room. Layers in order; each hands a cleaner artifact to the
next. The reviewers are personas an agent can run, but the COLD LISTENER works best as
a genuinely fresh context (a subagent or a person given the artifact and nothing else) —
the lesson of the evidence room's readability crisis applies here squared: the system
must contain someone encountering the story for the first time, and a listener cannot
even re-read.

## Layer 1 — Structural audit (mechanical, run first)
...
- First `COULD` lands inside the opening 10% of the runtime, or inside the declared
  ...
  action list; nothing after `BLISS` except the Big Idea restated, including after
  Q&A (re-close).
- One unrepeated-peak `STAR` — or two when the slot exceeds 60 minutes OR the
  working file declares two registers (one per register) — placed after the midpoint
  and not inside the ask. A line said three or more times is an anchor, exempt from
  the count.
- The run-of-show itself, mechanically: the clock's beat times sum to the declared
  content length (~two-thirds of the slot); every beat locked anywhere in the
  working file appears in the clock exactly once; the post-Q&A re-close is a clock
  line, not an intention. (Two sections each claiming slide 2, or a locked beat
  absent from the clock, is a thirty-second check no human reviewer reliably makes.)
- Every load-bearing beat has a `say-verbatim` sound bite ≤ 20 words.
- THREE NUMBERS block filled, and the cap governs RETENTION: at most three numbers
  recur across beats as remember-this repetitions. Structural numbers (grids, print
...
2. **Drifted** — traceable but restated loosely (departing from BOTH lines of the
   ...
   guard line is deliberate approximation and is NOT drift — drift is an approximation
   the card never approved, or a departure from the EXACT line where the exact figure
   is being quoted. Restore the guarded form.
3. **Bent** — the story's frame contradicts the derivation ("everything international",
   "never moved"). This is the firing offense: the beat is rewritten around the true
   shape, and the reviewer checks whether the Big Idea itself leaned on the bend.
...
**Precondition: a frozen setlist.** A beat library, a menu of arcs, or a working file
with options still open is not a cold-listener input — the listener would be
auditing a document, not hearing a talk. Freeze the run-of-show into one linear
script or deck first; if the setlist cannot be frozen yet, this layer is BLOCKED,
not skipped, and the gauntlet is incomplete until it runs.

Duarte's own version is the screening: a small panel profiled like the target audience,
deliberately including naysayers, budgeted at **three times the talk's length**
(*Resonate* p. 182 — a 20-minute talk gets a one-hour screening). Ours is leaner but
keeps her two invariants: the reviewer resembles the room, and sweetness disqualifies.

**Who the cold listener is, honestly.** The gold standard is a person who wasn't in
the build (John cold, a colleague, a family member for a public talk) — a human
genuinely cannot re-read a talk. An agent CANNOT run this layer on itself in the build
context; that is a rubber stamp. And even a fresh subagent is a weak proxy: it reads
with token attention, not listener memory — no sensory bottleneck, no decay, and a
bias toward reflecting back whatever is capitalized, repeated, or bolded. The
subagent variant therefore measures **salience, not memory**, and the protocol
prevents the questions from steering it: a fresh subagent gets the script/deck ONLY —
no sources, no working file, no conversation history, and NOT the questions below —
...
One reviewer plays the named hardest person in the actual room (gate 1 said who is
there — use them: the board member who hates spending, the engineer who checks math,
the rival program's champion). They hunt:
- the unpriced sacrifice (what the ask costs the room that the talk didn't say);
- the missing skeptic's apparatus (*DataStory* Ch. V requires it IN the artifact):
...
- the opposite case: could a competent skeptic argue the anti-Big-Idea from the same
  ...
  register: logical gets the counter-evidence, emotional gets the acknowledged value,
  practical gets the shouldered burden.
...
- **Build two endings** (*HBR Guide*): a false ending (the summary) and the real one
  (the new-bliss close), so running long costs you the summary, never the close.
- Practice against a clock counting UP while cutting; against a timer counting DOWN
  once inside the limit, with quarter-marks — know the beat you must be at when a
  40-minute slot hits 10, 20, 30.
- The sound bites are said VERBATIM at their beats — check against the cards.
- The IF ASKED answers exist for every beat (teach-back discipline) — the podium is
  where numbers bend, and the card is the anti-bend rail.
- The last sentence spoken is the Big Idea or its sound-bite form. Nothing follows it,
...
verdicts, what changed — in the project's DECISIONS.md or, for a one-off talk, at the
bottom of the working file itself; a gauntlet that leaves no trace decays into a
vibe.
